...
//function that will generate random temperature
function generateRandomNormalTemperature() {
    // Define the normal temperature range
    $minTemperature = 36.5;
    $maxTemperature = 37.6;

    // Generate a random temperature within the range
    $randomTemperature = rand($minTemperature * 10, $maxTemperature * 10) / 10.0;

    return $randomTemperature;
}

// Example usage
$temp = generateRandomNormalTemperature();
                        ...
                        if ($row['device_uid'] == $device_uid || $row['device_uid'] == 0){
                                ...
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    if (!$row = mysqli_fetch_assoc($resultl)){
                                        ...
                                        $temp = generateRandomNormalTemperature();
                                        $sql = "INSERT INTO users_logs (username, serialnumber, card_uid, temp, device_uid, device_dep, checkindate, timein, timeout) VALUES (? , ?, ?, '$temp', ?, ?, ?, ?, ?)";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
